		              ...
		              tree.cex=1.0)
{
  ...
  fpath <- function(fname) {
      return(paste(out_path, file.prefix, fname, sep="")
  }
  ...
  p <- spplot(spdf, zcol=trt[1], cex=0.25,
              ...
              #key=list(lines=TRUE),
              pch=c(2,5),
              main=list(label=paste("Treatment (",trt[2],") and Control (",counterfactual.name,") Locations."), cex=0.5),
  ...
  # ============================================================
  # Pre-balance plot - propensity score and four key variables
  # (first four in list taken otherwise; prefix_prebalance.png)

  pre.balance.path <- fpath("prebalance.png")
  plot.dta <- sub.dta
  ...
  prop.model.out <- fpath("propensityModel.html")
  ...
  # ============================================================
  # Post-balance plot - propensity score and four key variables
  # (first four in list taken otherwise; prefix_postbalance.png)

  post.balance.outpath <- fpath("postbalance.png")
  ...
  balance.stats.out <- fpath("balancestats.html")
  ...
  write.csv(final.tree$frame, fpath("tree_frame.csv"))
  write.csv(final.tree$splits, fpath("tree_splits.csv"))

  sink(fpath("tree_overview.txt"))
  print(final.tree)
  sink()

  save(final.tree, file=fpath("tree_data.rdata"))
  ...
  linear.het.out <- fpath("linearMatch.html")
  ...
  out.sum <- fpath("summary_estimates.txt")

  print("SUMMARY STATISTICS OF TREE PREDICTION:")
  print(summary(sub.dta$tree.pred))

  write("Treatment Cases:", file=out.sum, append=TRUE)
  write(paste(dim(sub.dta.desc[sub.dta.desc[trt[1]] == 1,])[1]), file=out.sum, append = TRUE)
  write("Control Cases:", file=out.sum, append=TRUE)
  write(paste(dim(sub.dta.desc[sub.dta.desc[trt[1]] == 0,])[1]), file=out.sum, append=TRUE)
  ...
  # write trt only
  # write.csv(trt.dta, fpath("prediction.csv"))
  # write trt and cnt

  write.csv(sub.dta, fpath("prediction.csv"))

  # Biggset and smallest
  ret.trt.dta <- trt.dta[trt.dta$tree.pred == max(trt.dta$tree.pred) |
                           trt.dta$tree.pred == min(trt.dta$tree.pred),]

  best.worst.out <- fpath("BW.csv")
  write.csv(ret.trt.dta, best.worst.out)
  ...
  rf.res <- read.csv(fpath("rf.csv"), header=FALSE)
  ...
  print(html_out)
  write(html_out, file=fpath("complete.html"))
